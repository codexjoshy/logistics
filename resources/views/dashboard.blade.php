...
<div class="row">
    <div class="col-12">
        ...
        <x-base.card title="Dashboard">
            @if (auth()->user()->company && optional(auth()->user())->company->status == 'verified')
                    <p>Company Link: <a target="_blank" href="{{$url}}"><strong>{{$url}}</strong></a></p>
                <p>
                    As a verified Registered Operator on this platform, click on the link below to join the operators whatsap group get our latest information and support 
                </p>
                <p><a target="_blank" href="{{$groupUrl}}">Click to Join Operator Whatsap Group</a></p>
                
            @else
            <h3>Your Registration is currently in {{optional(auth()->user()->company)->status ?? 'pending'}} </h3>
            <p>Click the button below to update your profile in order to get verified</p>
            <center>
                <a href="{{route('company.profile.create')}}"> Update Profile</a>
            </center>
            @if(optional(auth()->user()->company)->status == 'review') 
                ...
                <x-modal title="Review" key="review" data-backdrop="static" openOnFieldError="review_message">
                    <p>{{optional(auth()->user()->company)->reason}}</p>
                </x-modal>
                @endif
            @endif
        </x-base.card>
